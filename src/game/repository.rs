...
const COLLECTION: &str = "game";

pub fn all(connection: &Conn) -> Result<Vec<Game>, Error> {
...
pub fn get(id: ObjectId, connection: &Conn) -> Result<Option<Game>, Error> {
    match connection
...
pub fn insert(game: Game, connection: &Conn) -> Result<ObjectId, Error> {
    let insertable = InsertableGame::from_cat(game.clone());
...
pub fn update(id: ObjectId, game: Game, connection: &Conn) -> Result<Game, Error> {
    let mut new_cat = game.clone();
